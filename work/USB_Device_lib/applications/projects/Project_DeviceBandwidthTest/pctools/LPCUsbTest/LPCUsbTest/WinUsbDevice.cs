...
namespace LPCTest
{
	...
	sealed internal partial class WinUsbDevice
	{
		...
		internal struct devInfo
		{
            ...
            internal int outPipes;
            internal int inPipes;
		...
		internal Boolean Do_Control_Read_Transfer(ref Byte[] dataStage)
		{
			...
			try
			{
				...
				success = WinUsb_ControlTransfer(myDevInfo.winUsbHandle, setupPacket, dataStage, System.Convert.ToUInt16(dataStage.Length), ref bytesReturned, IntPtr.Zero);
				return success;
		...
		internal Boolean InitializeDevice()
		{
			...
			try
			{
				...
				if (success)
				{
					...
					if ((success) && (ifaceDescriptor.bNumEndpoints > 1))
					{
                        // now allocate ep arrays.
                        if (myDevInfo.bulkInPipe == null)
                        {
                            myDevInfo.bulkInPipe = new Byte[ifaceDescriptor.bNumEndpoints];
                            myDevInfo.bulkOutPipe = new Byte[ifaceDescriptor.bNumEndpoints];
                        }
                        myDevInfo.inPipes = myDevInfo.outPipes = 0;
						...
						for (Int32 i = 0; i <= ifaceDescriptor.bNumEndpoints - 1; i++)
						{
                            ...
                            WinUsb_FlushPipe(myDevInfo.winUsbHandle, pipeInfo.PipeId);

                            
							if (pipeInfo.PipeType == USBD_PIPE_TYPE.UsbdPipeTypeBulk)
                            {
                                if (UsbEndpointDirectionIn(pipeInfo.PipeId))
                                {
                                    myDevInfo.bulkInPipe[myDevInfo.inPipes++] = pipeInfo.PipeId;
                                }
                                else
                                {
                                    myDevInfo.bulkOutPipe[myDevInfo.outPipes++] = pipeInfo.PipeId;
                                }
		...
		///  
		internal void ReadViaBulkTransfer(ref Byte pipeID, UInt32 bytesToRead, ref Byte[] buffer, ref UInt32 bytesRead, ref Boolean success)
		{
			try
			{
				// ***
		...
		internal Boolean SendViaBulkTransfer(Byte pipeID, ref Byte[] buffer, UInt32 bytesToWrite)
		{
        ...
        {
            return WinUsb_FlushPipe(myDevInfo.winUsbHandle, pipeID);
        }
